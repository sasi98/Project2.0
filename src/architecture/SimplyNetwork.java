...
public class SimplyNetwork {
		...
		{	
			...
			if ( (W.getRow()  == numNeuronsS) && (W.getColumn() == valuesInputLayer.length) ){
				...
				log.error("Las dimensiones de W no coinciden con el número de neuronas del patr�n \n");			
			}
		}
				...
				if ( (W.getRow()  == numNeuronsS) && (W.getColumn() == numNeuronsE) ){
			        ...
			        for (int i = 0; i < outputLayer.length; i++) {
			            for (int j = 0; j < inputLayer.length; j++) {
			                // Create the connection object and put it in both neurons, and give it the weight from the matriz W            	
			                Connection c = new Connection(inputLayer[j],outputLayer[i],W.getValuePos(i, j));
			                log.debug("Creando conexi�n: From: "+ inputLayer[j] +" to " + outputLayer[i] + " con peso " + W.getValuePos(i, j));
			                this.outputLayer[i].addConnection(c);
			                this.inputLayer[j].addConnection(c);
			            }
			        }
					...
					log.error("Las dimensiones de W no coinciden con el n�mero de neuronas del patr�n \n");
				}
			}
...
//			writer.closeFile();
	      ...
	      //Calculo los deltas de error de la capa de salida
	        ...
	        //Actualizaci�n de matriz: C�lculo de deltaW
